...
def initialize_camera():
    ...
    with camera_lock:
        ...
        print("Error: Failed to initialize camera on any index")
        return False
...
def stop_motor():
    ...
    # Don't release camera here - let it be managed by capture functions
    # Just ensure it's in a good state
    with camera_lock:
        if not camera.isOpened():
            initialize_camera()
...
# Flask Routes
# (send_sms, send_image_to_owner, get_location, stream_logs, etc.)
