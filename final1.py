...
start_event = threading.Event()  # Event to signal start completion
        ...
        for i in range(3):
            ...
            if camera.isOpened():
                ...
                if ret and frame is not None and frame.size > 0:
                    print(f"Camera test capture successful on index {i}")
                    return True
                else:
                    print(f"Camera test capture failed on index {i}")
            sleep(1)
    ...
    with camera_lock:
        while retry_count < max_retries:
            ...
            if face_locations:
                ...
                current_time = datetime.now()
                if (current_time - last_log_time).total_seconds() > 1:
                    new_log = f"{current_time.strftime('%H:%M:%S')} - Image captured successfully"
                    server_logs.append(new_log)
                    last_log_time = current_time
...
def start_vehicle_with_face():
    global motor_running, vehicle_stopped, server_logs, last_log_time, pending_authorization, sms_sent_for_current_attempt, start_event
    if vehicle_stopped:
...
def start_motor_web():
    global server_logs, last_log_time, start_event
    print("Starting vehicle via web...")
    start_event.clear()
    threading.Thread(target=start_vehicle_with_face).start()
    # Wait for the start event to be set (timeout after 35 seconds)
    if not start_event.wait(timeout=35):
        print("Timeout waiting for face recognition to complete")
        current_time = datetime.now()
        if (current_time - last_log_time).total_seconds() > 1:
            new_log = f"{current_time.strftime('%H:%M:%S')} - Timeout waiting for face recognition"
            server_logs.append(new_log)
            last_log_time = current_time
    return jsonify({"logs": server_logs})
