...
def capture_image_with_face():
    ...
    max_retries = 5  # Increased retries to ensure new capture
    retry_count = 0
    ...
    with camera_lock:
        while retry_count < max_retries:
            if not camera.isOpened():
                if not reinitialize_camera():
                    print("Error: Camera reinitialization failed after retries")
            ...
            # Flush camera buffer with multiple grabs
            for _ in range(3):
                camera.grab()
            ...
            # Compute hash of the frame to check for uniqueness
            frame_hash = hashlib.md5(frame.tobytes()).hexdigest()
            if last_frame_hash == frame_hash:
                print("Same frame detected. Waiting for new frame...")
                sleep(0.5)
                continue
            ...
            if face_locations:
                ...
                if success:
                    ...
                    current_time = datetime.now()
                    if (current_time - last_log_time).total_seconds() > 1:
                        new_log = f"{current_time.strftime('%H:%M:%S')} - New image captured: {captured_image_path}"
                        server_logs.append(new_log)
                        last_log_time = current_time
        ...
        current_time = datetime.now()
        if (current_time - last_log_time).total_seconds() > 1:
            new_log = f"{current_time.strftime('%H:%M:%S')} - Max retries reached, no valid image captured"
            server_logs.append(new_log)
            last_log_time = current_time
        return None
...
def start_vehicle_with_face():
    global motor_running, vehicle_stopped, server_logs, last_log_time, pending_authorization, sms_sent_for_current_attempt, latest_captured_image
    if vehicle_stopped:
        ...
        # Reset latest_captured_image to ensure a fresh image is required
        ...
        while not captured_image_path and vehicle_stopped:
            ...
            if not captured_image_path:
                ...
                latest_captured_image = captured_image_path  # Update only on success
                break

        if not captured_image_path:
            current_time = datetime.now()
            if (current_time - last_log_time).total_seconds() > 1:
                new_log = f"{current_time.strftime('%H:%M:%S')} - No valid image captured, aborting attempt"
                server_logs.append(new_log)
                last_log_time = current_time
            return
...
def send_sms():
    ...
    try:
        r = requests.post(
            ...
            data={
                'auth_token': 'a5a492a8f95c9a887b799021d5a447890d9d7bf1e5eeb954b38ebfa9cf76b482mm',
                'to': '9818858567',
                'text': message
            },
