...
def compare_faces(captured_image_path):
    try:
        ...
        for image_path, ref_encoding in reference_encodings:
            ...
            if results[0]:
                print(f"Face match with {os.path.basename(image_path)}")
                return True
        return False
    except Exception as e:
...
def start_vehicle_with_face():
    ...
    # Only proceed if face recognition is active (triggered by start button)
    ...
    if vehicle_stopped:
        print("Waiting for IR sensor...")
        current_time = datetime.now()
        new_log = f"{current_time.strftime('%H:%M:%S')} - Waiting for IR sensor"
        server_logs.append(new_log)
        last_log_time = current_time

        while IR_SENSOR.value:
            if not face_recognition_active:  # Check if we should abort
                return
            sleep(0.1)

        print("IR sensor detected, starting face recognition")
        current_time = datetime.now()
        new_log = f"{current_time.strftime('%H:%M:%S')} - Starting face recognition"
        server_logs.append(new_log)
        last_log_time = current_time

        captured_image_path = None
        while not captured_image_path and vehicle_stopped and face_recognition_active:
        ...
        if match_found:
            ...
            print("No face match, requesting authorization")
