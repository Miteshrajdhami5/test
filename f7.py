...
def capture_image_with_face():
    ...
    max_retries = 10
    retry_count = 0
    min_face_size = 100  # Reduced from 150 to allow more distance (pixels)
    max_face_size = 300  # Added maximum face size
    min_brightness = 50
    optimal_face_ratio = 0.2  # Face should occupy 20-40% of frame height
    ...
    with camera_lock:
        ...
        while retry_count < max_retries:
            ...
            if brightness < min_brightness:
                print(f"Image too dark ({brightness:.1f}) - improve lighting")
                sleep(0.5)
                continue

            # Detect faces with better parameters for distance
            rgb_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
            face_locations = face_recognition.face_locations(
                rgb_frame,
                model="hog",
                number_of_times_to_upsample=1  # Better for distant faces
            )
            
            if not face_locations:
                print("No face detected - ensure clear view of face")
                sleep(0.5)
                continue
            ...
            face_ratio = face_height / frame.shape[0]  # Ratio of face height to frame height
            
            # Size checks - allow smaller faces but with stricter quality controls
            if face_width < min_face_size or face_height < min_face_size:
                print(f"Face too small ({face_width}x{face_height}) - move slightly closer")
                sleep(0.5)
                continue
                
            if face_width > max_face_size or face_height > max_face_size:
                print(f"Face too large ({face_width}x{face_height}) - move slightly back")
                sleep(0.5)
                continue
                
            # Face position and quality checks
            ...
            if x_offset > 0.3 or y_offset > 0.3:  # Slightly more lenient on position
                print("Please center your face in the frame")
                sleep(0.5)
                continue
                
            # Check face ratio (not too small/big in frame)
            if face_ratio < 0.15:  # Face should be at least 15% of frame height
                print("Please move slightly closer - face too small in frame")
                sleep(0.5)
                continue
                
            if face_ratio > 0.4:  # Face shouldn't be more than 40% of frame height
                print("Please move slightly back - face too large in frame")
                sleep(0.5)
                continue
                
            # Enhanced quality check using face landmarks
            face_landmarks = face_recognition.face_landmarks(rgb_frame, [(top, right, bottom, left)])
            if face_landmarks:
                # Check for visible facial features
                required_features = ['chin', 'left_eye', 'right_eye', 'nose_bridge']
                if not all(feat in face_landmarks[0] for feat in required_features):
                    print("Face features not clear - ensure good visibility")
                    sleep(0.5)
                    continue
                ...
                if eye_distance < 30:  # Eyes too close together (too far/face too small)
                    print("Please move slightly closer - eyes not visible clearly")
                    sleep(0.5)
                    continue

            # Save the high-quality image
            cv2.imwrite(captured_image_path, frame)
            print(f"Captured good quality image from distance: {captured_image_path}")
            print(f"Face size: {face_width}x{face_height} (ratio: {face_ratio:.2f})")
