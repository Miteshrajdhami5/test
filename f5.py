...
for image_path in reference_image_paths:
    try:
        ...
        encodings = face_recognition.face_encodings(image, num_jitters=20, model="cnn")
        if encodings:
    ...
    with camera_lock:
        ...
        for i in range(3):
            ...
            if camera.isOpened():
                ...
                for _ in range(10):  # Test capture to ensure stability
...
# Modified image capture function
def capture_image_with_face():
    ...
    with camera_lock:
        ...
        while retry_count < max_retries:
            ...
            rgb_frame = preprocess_image(frame)
            ...
            if face_width < 100 or face_height < 100:
                print("Face too small - please move closer to camera")
                retry_count += 1
                sleep(0.5)
                continue
            ...
            if os.path.exists(captured_image_path):
                ...
                if captured_image is None:
                    print(f"Error: Failed to load captured image {captured_image_path}")
                    retry_count += 1
                    continue
...
# Modified face comparison function
def compare_faces(captured_image_path):
    ...
    try:
        ...
        captured_encodings = face_recognition.face_encodings(captured_image, num_jitters=20, model="cnn")
        if not captured_encodings:
            print(f"Error: No face found in captured image {captured_image_path}")
            return False

        captured_encoding = captured_encodings[0]
        print(f"Captured encoding shape: {captured_encoding.shape}")
        ...
        for image_path, ref_encoding in reference_encodings:
            ...
            print(f"Reference encoding shape: {ref_encoding.shape}")
            face_distance = face_recognition.face_distance([ref_encoding], captured_encoding)[0]
            confidence = (1 - face_distance) * 100
            print(f"Distance: {face_distance:.4f}, Confidence: {confidence:.1f}%")
            if face_distance < best_distance:
                best_distance = face_distance
                best_confidence = confidence
                best_match_path = image_path

        if best_distance < 0.6 and best_confidence > 70:
            print(f"Match found with {os.path.basename(best_match_path)} (distance: {best_distance:.4f}, confidence: {best_confidence:.1f}%)")
            current_time = datetime.now()
            server_logs.append(f"{current_time.strftime('%H:%M:%S')} - Verified match (confidence: {best_confidence:.1f}%)")
            last_log_time = current_time
            return True
        else:
            print(f"No match (best distance: {best_distance:.4f}, confidence: {best_confidence:.1f}%)")
            server_logs.append(f"{datetime.now().strftime('%H:%M:%S')} - No match found")
            return False
        
    except Exception as e:
        print(f"Error in face comparison: {e}")
        server_logs.append(f"{datetime.now().strftime('%H:%M:%S')} - Face comparison error: {e}")
        return False
...
def start_vehicle_with_face():
    ...
    with camera_lock:
        if not camera.isOpened():
            if not initialize_camera():
                server_logs.append(f"{datetime.now().strftime('%H:%M:%S')} - Camera initialization failed")
                return

    if vehicle_stopped:
        current_time = datetime.now()
        server_logs.append(f"{current_time.strftime('%H:%M:%S')} - Starting face verification")
        last_log_time = current_time

        print("Waiting for IR sensor (finger) detection...")
        while IR_SENSOR.value and face_recognition_active:
            sleep(0.1)

        captured_image_path = capture_image_with_face()
        if not captured_image_path:
            server_logs.append(f"{datetime.now().strftime('%H:%M:%S')} - Failed to capture valid image")
            return
        ...
        if match_found:
            ...
            try:
                ...
                print(f"Deleted temporary image: {captured_image_path}")
            except Exception as e:
...
def send_sms():
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            server_logs.append(f"{datetime.now().strftime('%H:%M:%S')} - SMS sent successfully")
        else:
            print(f"Failed to send SMS: {r.text}")
            server_logs.append(f"{datetime.now().strftime('%H:%M:%S')} - Failed to send SMS")
    except Exception as e:
        print(f"Error sending SMS: {e}")
        server_logs.append(f"{datetime.now().strftime('%H:%M:%S')} - Error sending SMS: {e}")
...
def send_image_to_owner(captured_image_path):
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            server_logs.append(f"{current_time.strftime('%H:%M:%S')} - Unauthorized access detected, SMS sent")
        else:
            print(f"Failed to send SMS: {r.text}")
            server_logs.append(f"{current_time.strftime('%H:%M:%S')} - Failed to send SMS")
        last_log_time = current_time
    except Exception as e:
        print(f"Error sending SMS: {e}")
        server_logs.append(f"{current_time.strftime('%H:%M:%S')} - Error sending SMS: {e}")
        last_log_time = current_time
...
def get_location():
    ...
    if (current_time - last_log_time).total_seconds() > 1:
        server_logs.append(f"{current_time.strftime('%H:%M:%S')} - Using fixed location: {fixed_location}")
        last_log_time = current_time
...
def authorize():
    ...
    if request.method == 'POST':
        ...
        if action == 'yes':
            ...
            if (current_time - last_log_time).total_seconds() > 1:
                server_logs.append(f"{current_time.strftime('%H:%M:%S')} - Owner authorized via web! Starting vehicle...")
                last_log_time = current_time
            ...
            if (current_time - last_log_time).total_seconds() > 1:
                server_logs.append(f"{current_time.strftime('%H:%M:%S')} - Owner denied access via web.")
                last_log_time = current_time
