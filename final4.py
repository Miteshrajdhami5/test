...
def start_vehicle_with_face():
    ...
    while attempt < max_attempts and vehicle_stopped:
        ...
        sms_sent_for_current_attempt = False
        print(f"Attempt {attempt + 1}/{max_attempts} to start vehicle...")
        current_time = datetime.now()
        if (current_time - last_log_time).total_seconds() > 1:
            new_log = f"{current_time.strftime('%H:%M:%S')} - Attempt {attempt + 1}/{max_attempts} to start vehicle"
            server_logs.append(new_log)
            last_log_time = current_time
...
def send_sms():
    website_url = 'http://192.168.80.17:5000'
    message = f"""
...
def send_image_to_owner(captured_image_path):
    ...
    current_time = datetime.now()
    image_url = f"http://192.168.80.17:5000/captured_image"
    message = f"""
    🚨 Unauthorized Access Attempt!!!

    An unknown person tried to start the vehicle.
    Image: {image_url}
    Check authorization on: http://192.168.80.17:5000
    """
    try:
        r = requests.post(
            "https://sms.aakashsms.com/sms/v3/send/",
...
def index():
    ...
    start_url = 'http://192.168.80.17:5000/start_vehicle'
    stop_url = 'http://192.168.80.17:5000/stop_vehicle'
    location_url = 'http://192.168.80.17:5000/send_location'
    map_redirect_url = 'http://192.168.80.17:5000/redirect_to_map'
