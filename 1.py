    ...
    with camera_lock:
        ...
        for i in range(3):
            ...
            if camera.isOpened():
                ...
                log_message = f"Camera initialized on index {i}"
                print(log_message)
                server_logs.append(f"{datetime.now().strftime('%H:%M:%S')} - {log_message}")
                sleep(1)
                return True
        log_message = "Error: Failed to initialize camera"
        print(log_message)
        server_logs.append(f"{datetime.now().strftime('%H:%M:%S')} - {log_message}")
        return False
...
if not initialize_camera():
    log_message = "Failed to initialize camera initially"
    print(log_message)
    server_logs.append(f"{datetime.now().strftime('%H:%M:%S')} - {log_message}")
    exit()
...
def start_vehicle_with_face():
    ...
    with camera_lock:
        if not camera.isOpened():
            if not initialize_camera():
                log_message = "Failed to reinitialize camera for face verification"
                print(log_message)
                server_logs.append(f"{datetime.now().strftime('%H:%M:%S')} - {log_message}")
                return
    if vehicle_stopped:
